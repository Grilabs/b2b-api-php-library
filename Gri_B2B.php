...
class griB2B
{
    ...
    )
    {
        ...
        $this->baseUrl = ($test_mode) ? self::API_URL_SANDBOX : self::API_URL_PROD;
        ...
        try {
            ...
            if($this->debugMode) {
                $this->logger->log([
                    ...
                    'url' => $urlCompleted,
                    'method' => $method,
                    'headers' => $headersReadyToSent,
                    'data' => $paramsCompleted,
                    'response' => $body
                ]);
            }
    ...
    public function get_token($force_refresh = false, $force_new_token = false)
    {
        ...
        if (!$force_new_token && (($force_refresh) || isset($contentObj->refresh_token) && $contentObj->refresh_token)) {
            try {
                ...
                $this->logger->log("Error occurred while refreshing token! Error message: " . $exception->getMessage());
            }
        }
        ...
        if (is_null($res)) {
            try {
                ...
                $this->logger->log("Error occurred while create new token! Error message: " . $exception->getMessage());
            }
        }
